...
4p. **Proximity preferences (optional)** If the user has set proximity_preferences (in stored preferences or stated in chat), apply them before or after the initial present: (1) Call parse_proximity_preferences(proximity_text) with the stored or stated text. (2) Call geocode_proximity_references(rules) with the returned rules. (3) Call enrich_listings_with_proximity(listings, rules, geocoded_refs) with the current listings, the rules, and the refs from step 2. (4) Call filter_listings with proximity_rules=rules (and no other criteria if only applying proximity) so that listings satisfy ALL rules (AND). Listings without coordinates or with unknown proximity for a rule are kept and shown as "distance unknown". When presenting results that have proximity data, add a **Proximity** bullet to the summary (e.g. "N listings match your proximity criteria; M have distance unknown for one or more criteria."). The UI may show per-rule distance/duration or "Distance unknown" for each listing.
