...
4. **Present** In the UI, results are shown in a table (rank, MLS id, address, bed, bath, size, rent, URL). You MUST ALWAYS call summarize_listings first, then produce a **bullet-point summary** with one bullet per parameter: Count, Price, Bedrooms, Bathrooms, Size (if available), Property types (if available). Each bullet should contain human-readable wording (not raw stats). Example format:
